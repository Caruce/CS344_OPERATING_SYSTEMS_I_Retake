...
fn main() {
    ...
    for i in 0..data_size {
      let mut thread_data = vec![0; xs[i].len()];
      for j in 0..xs[i].len() {
        thread_data[i] = xs[i][j]
      }
      data_arr.push(thread_data)
    }
    // init thread
    for i in 0..data_size {
      let tdata = data_arr[i].drain(0..).collect();
      let thdl = thread::spawn(move || {
        map_data(&tdata)
      });
      thread_arr.push(thdl);
    }

    // wait threads
    let mut titer = thread_arr.into_iter();
    while let Some(t) = titer.next() {
      let map_value = t.join().unwrap();
      intermediate_sums.push(map_value);
    }
    ...
    let multiple_thread_sum = reduce_data(&intermediate_sums);
    // 6. Prints the final sum computed by reduce_data
    println!("Sum = {}", multiple_thread_sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
  let one_partition_len = v.len() / num_partitions;
  let mut left_size = 0;
  if 0 != v.len() % num_partitions {
    left_size = v.len() - one_partition_len * num_partitions;
  };

  let mut partitioned_data : Vec<Vec<usize>> = Vec::new();

  // calculate every partition size
  for _ in 0..num_partitions {
    let mut tmp_partition_len = one_partition_len;
    if 0 < left_size {
      tmp_partition_len += 1;
      left_size -= 1;
    }

    let tmp_data = vec![0; tmp_partition_len];
    println!("{}", tmp_data.len());
    partitioned_data.push(tmp_data);
  }

  // fill partition with data
  let mut cur_pos = 0;
  for j in 0..partitioned_data.len() {
    let item = & mut partitioned_data[j];
    for k in 0..item.len() {
      item[k] = v[cur_pos + k];
    }
    cur_pos +=  item.len();
