...
int ENC_Is_OK(char *pInput)
{
	...
	{
		if (!((pInput[nindex]<='Z' && pInput[nindex]>='A') || pInput[nindex] == ' '))
		{
			return -1;
		}
	}
...
int main(int argc, char **argv)
{
	...
	nkeylen = FILE_GetFileSize(akey_path);
	
	if (nkeylen < nfile_len)
	{
    ...
    FILE_ReadData(afile_path, 2, 1000, 1, atmp_buf, 0);
	nis_ok = ENC_Is_OK(atmp_buf);
	if (nis_ok < 0)
	{
	...
	//printf("key begin \n");
    psend_data = (char*)malloc(nkeylen+sizeof(struct SHeader));
    memset(psend_data, 0x0, nkeylen+sizeof(struct SHeader));
    phead = (struct SHeader*)psend_data;
	//printf("key file len = %d \n", nkeylen);
	
	phead->m_nIsOk = 200;
	phead->n_ntype = 2;
	phead->m_nBodyLen = nkeylen;
	phead->m_nCurrIndex = 1;
	phead->m_nItemSize = 1000;
	phead->m_nItemCount =  nkeylen/1000 + 
									((nkeylen%1000 > 0)?1:0);
	//printf("key send begin header file len = %d, phead->m_nItemCount = %d\n", nkeylen, phead->m_nItemCount);
	nres = SOCKET_Send(nhandle, psend_data, sizeof(struct SHeader));
	//free(psend_data);
	//printf("key send end header realse len = %d \n", nres);
	
    for (nindex = 0; nindex < phead->m_nItemCount; nindex++)
    {    
        memset(atmp_buf, 0x0, sizeof(atmp_buf));
        FILE_ReadData(akey_path, 2, 1000, 1, atmp_buf, 1000*(phead->m_nCurrIndex - 1));
		//printf("key key send data = %s , path = %s, index = %d\n", atmp_buf, akey_path, phead->m_nCurrIndex);
        nres = SOCKET_Send(nhandle, atmp_buf, strlen(atmp_buf));
		//printf("key key send data len = %d. src = %s \n", nres, atmp_buf);
		phead->m_nCurrIndex++;
    }
    //free(psend_data);
    ...
    for (;;)
    {
		...
		phead = (struct SHeader*)phead_data;
		//free(psend_data);
		//printf("ok = %d, type = %d, len = %d\n", phead->m_nIsOk, phead->n_ntype, nres);
		if (phead->m_nIsOk == 300)
		{
			fprintf(stderr, "Error: could not contact enc_server on port %d\n", port);
			close(nhandle);
