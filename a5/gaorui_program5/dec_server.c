...
/***********************************************************
* Function    : SOCKET_PrintfServerVersion
...
/***********************************************************
* Function    : ENC_SocketListen
* Description : Listen on sockets
* Input       : port
* Output      : no
* Return      : no
* Date        : 2021.05.28
    ...
    for(;;)
    {
        ...
        if((child_pid = fork())==0)
        {
		    for (;;)
			{
				...
				{
					cipherLen = strlen(prcv_file_data) - 1;
					keyLen = strlen(prcv_key_data) -1 ;
					//printf("cipherLen = %d, keyLen = %d\n", cipherLen, keyLen);
					for(i = 0; i < keyLen; i++)
					{
						if(i >= cipherLen)
						{
						...
						cipherSeq = get_seq(prcv_file_data[i]);
						keySeq = get_seq(prcv_key_data[i]);
						//printf("---000-cipherSeq = %d, keySeq= %d\n", cipherSeq, keySeq);
						//calculate the cipher sequence
						plainSeq = cipherSeq - keySeq;
						//printf("---001-plainSeq = %d\n", plainSeq);
						if(plainSeq < 0)
						{
								plainSeq = (plainSeq + 27) % 27;
						}else{
								plainSeq = plainSeq % 27;
						}
						//from sequence to letter
						//printf("---002-plainSeq = %d\n", plainSeq);
						ciphertextBuf[i] = get_letter(plainSeq);
						//printf("get min wen char %c\n", ciphertextBuf[i]);
					}
					ciphertextBuf[++i] = '\0';
					//printf("key .. src = %s\n", ciphertextBuf);	
					
					//printf("ntotal_type = %d \n", ntotal_type);					
